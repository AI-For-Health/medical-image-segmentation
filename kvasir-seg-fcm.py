...
from image_segmenters import FCM
import numpy as np
...
for idx in range(len(dataset["validation"]["image"])):
    fcm=FCM(n_clusters=2, m=2, max_iter=100, error=1e-5, random_state=42)
    X=preprocess(dataset["validation"]["image"][idx]).reshape(-1,1)
    fcm.fit(X)
    labels=fcm.predict(X).reshape(np.array(dataset["validation"]["image"][idx]).shape[:2])
    dice_scores.append(binary_dice_score(np.array(dataset["validation"]["annotation"][idx]), labels))
    # print(dice_scores[-1])
    pbar.update(1)
